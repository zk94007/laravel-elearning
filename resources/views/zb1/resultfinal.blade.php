<?php 
	$resultLesen = $viewHelper->calculateAndDisplayTestResult('l');
	$resultHoeren = $viewHelper->calculateAndDisplayTestResult('h');
	$allResult = ($resultLesen+$resultHoeren)/2;
	$badgeColor = 'bronce';
	if($allResult > 60)
		$badgeColor = 'silver';
	if($allResult > 85)
	...
	$resultbarColorLesen = 'danger';
	if($resultLesen > 60 )
		$resultbarColorLesen = 'warning';
	if($resultLesen > 85 )
		$resultbarColorLesen = 'success';
        ...
        <div class="large-12-columns">
          ...
          <div class="badge-container {{$badgeColor}}" id="badge">
            ...
            <div class="txt-container layer" data-depth="0.25">
              ...
              <div class="percentage-display">{{ $allResult }}%</div>
            </div>
          </div>
        </div>
      	<?php } else {?>
      		<h2>Zu wenig Punkte</h2>
      		<p> Du hast insgesamt leider zu wenige Fragen richtig beantwortet. <br/>
      		Versuch es noch einmal, in dem Du den Beenden-Button klickst und den Test noch einmal startest!</p>
      	<?php } ?>
      </div>
      <p>
      <h5 class="text-center">Auswertung Lesen</h5>
      		<p>Du hast <b>{{ $resultLesen }}</b> von 100 möglichen Punkten in dem Modul Lesen erreicht
      		</p><br/>
      		<p>Du hast <b>{{ $resultHoeren }}</b> von 100 möglichen Punkten in dem Modul Hören erreicht
      		</p><br/>
      </p>
      <h5 class="text-center">Auswertung</h5>
      <div class="row medium-up-2">
        <div class="column column block">
          <span class="label">Lesen</span>
          <div class="progress" role="progressbar" tabindex="0" aria-valuenow="50" aria-valuemin="0" aria-valuetext="50 percent" aria-valuemax="100">
            <div class="progress-meter {{$resultbarColorLesen}}" style="width: {{ $resultLesen }}%"></div>
          </div>
        </div>
        <div class="column column block">
        ...
        <a onClick="loadQuestion('zb1',16);" class="nav-button prev" data-equalizer-watch>
