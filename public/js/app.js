...
"use strict";
/* WEBPACK VAR INJECTION */(function(module) {var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;
...
(function webpackUniversalModuleDefinition(root, factory) {
	if (( false ? "undefined" : _typeof2(exports)) === 'object' && ( false ? "undefined" : _typeof2(module)) === 'object') module.exports = factory();else if (true) !(__WEBPACK_AMD_DEFINE_ARRAY__ = [], __WEBPACK_AMD_DEFINE_FACTORY__ = (factory),
				...
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));else if ((typeof exports === "undefined" ? "undefined" : _typeof2(exports)) === 'object') exports["whatInput"] = factory();else root["whatInput"] = factory();
})(this, function () {
...
!function (t, i, e) {
	...
	function s(t, i) {
		...
		this.transform3DSupport && this.accelerate(this.element);var i = t.getComputedStyle(this.element);"static" === i.getPropertyValue("position") && (this.element.style.position = "relative"), this.updateLayers(), this.updateDimensions(), this.enable(), this.queueCalibration(this.calibrationDelay);
...
$(function () {
	//badge parallax
	var scene = document.getElementById('badge');
	var parallax = new Parallax(scene);

	var scene = document.getElementById('badge-silver');
	var parallax1 = new Parallax(scene);

	var scene = document.getElementById('badge-bronce');
	var parallax2 = new Parallax(scene);

	// Only select one Checkbox for TRUE FALSE
	$('.item-true-false input[type=checkbox]').on('click', function (e) {
		$(this).prop('checked', true);

		var $parent = $(this).parents('.item'),
		    trueChkbx = $parent.find('.true input[type=checkbox]'),
		    falseChkbx = $parent.find('.false input[type=checkbox]');

		if ($(this).prop('id') == trueChkbx.prop('id')) {
			falseChkbx.prop('checked', false);
		} else {
			trueChkbx.prop('checked', false);
		}
	});

	// Only select one Checkbox for A B C
	$('.item-a-b-c input[type=checkbox]').on('click', function (e) {
		$(this).prop('checked', true);

		var $parent = $(this).parents('.item'),
		    aChkbx = $parent.find('.a input[type=checkbox]'),
		    bChkbx = $parent.find('.b input[type=checkbox]'),
		    cChkbx = $parent.find('.c input[type=checkbox]');

		switch ($(this).prop('id')) {
			case aChkbx.prop('id'):
				bChkbx.prop('checked', false);
				cChkbx.prop('checked', false);
				break;
			case bChkbx.prop('id'):
				aChkbx.prop('checked', false);
				cChkbx.prop('checked', false);
				break;
			case cChkbx.prop('id'):
				aChkbx.prop('checked', false);
	...
	var checkList = function checkList() {
		var selectedItems = [];
		$('.item-display').each(function (index, el) {
			selectedItems.push($(el).find('select').val());
		});

		$.each($('.display'), function (index, val) {
			var id = $(val).prop('id');
			if ($.inArray(id, selectedItems) > -1) {
				$('#' + id).addClass('disabled').prop('draggable', false);
			} else {
				$('#' + id).removeClass('disabled').prop('draggable', true);
			}
		});
	};

	$('.displays .display').on('dragstart', function (event) {
		event.originalEvent.dataTransfer.setData("text", event.target.id);
	});

	$('.item-display').on('dragover', function (event) {
		event.preventDefault();
	});

	$('.item-display').on('drop', function (event) {
		event.preventDefault();
		var data = event.originalEvent.dataTransfer.getData("text");

		// Disable Draggable Element
		$('.displays #' + data).prop('draggable', false).addClass('disabled');
		// set selectbox
	...
	$(".item-display select").on("change", function () {
		checkList();
	});
});

/*
	dragula([document.getElementById('displays'), document.getElementById('item-display')],{
		copy: function (el, source){
			return source === document.getElementById('displays');
		},
		accepts: function (el, target){
			return target !== document.getElementById('displays');
		}
	})
*/
